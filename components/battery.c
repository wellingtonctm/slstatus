 ...
 */
	...
	const char *
	battery_perc_custom(const char *bat)
	{
		int cap_perc;
		char cap_path[PATH_MAX], stt_path[PATH_MAX], state[12];;

		if (esnprintf(cap_path, sizeof(cap_path), POWER_SUPPLY_CAPACITY, bat) < 0)
			return NULL;
		if (pscanf(cap_path, "%d", &cap_perc) != 1)
			return NULL;
		if (esnprintf(stt_path, sizeof(stt_path), POWER_SUPPLY_STATUS, bat) < 0)
			return NULL;
		if (pscanf(stt_path, "%12[a-zA-Z ]", state) != 1)
			return NULL;
